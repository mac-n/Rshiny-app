cost_cfs<-function(formula, data, costs, lamda) {
  ...
  evaluator <- function(attrs) {
    ...
    #edit
    cost_mean =cost_sum /attr_count
    if(attr_count == 1)
      return(fc_cor - cost_mean*lamda)
    else
      #edited this line obviously
      return(attr_count * fc_cor / sqrt(attr_count + attr_count * (attr_count - 1) * ff_cor)-(cost_mean*lamda))
  }
  ...
  #added in this line so I can look up costs later. check attrnames are in same order as input
  
  names(costs)<-attr_names[2:attr_count]
  entropies = NULL
  if(classification) {
